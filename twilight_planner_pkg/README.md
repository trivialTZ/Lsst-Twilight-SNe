...
`L_det = lc_detections`, `L_exp = lc_exposure_s`. Let `\mathcal F` be the set of
filters used so far and `|\mathcal F|` its size. Define

$$
\mathrm{met}_\mathrm{H} := \mathbf{1}\Big( (N_{\rm det} \ge H_{\rm det} \wedge |\mathcal F| \ge 2)
\;\vee\; (T_{\rm exp} \ge H_{\rm exp}) \Big),\\
\mathrm{met}_\mathrm{L} := \mathbf{1}\Big( (N_{\rm det} \ge L_{\rm det})
...
$$
S = \mathbf{1}\big(\neg\,\mathrm{met}_\mathrm{H}\big)
\;\vee\; \Big( \mathbf{1}(\mathrm{Ia}) \wedge \mathbf{1}\big(\mathrm{met}_\mathrm{H}\big) \wedge \mathbf{1}\big(\neg\,\mathrm{met}_\mathrm{L}\big) \Big),
$$
...
S = \begin{cases}
1, & N_{\rm det}=0,\\[4pt]
S_{\rm resume}, & N_{\rm det}>0\ \wedge\ (\mathrm{now}-t_{\rm last}) > D_{\rm lookback},\\[4pt]
-1, & \text{otherwise.}
\end{cases}
$$
...
$$
V_{\mathrm{blue}},\ V_{\mathrm{red}} := \#\,\text{visits in }[\mathrm{now}-W,\,\mathrm{now}]\ \text{by group}.
$$

Let `T = color_target_pairs`. For a filter `f` in group
`\mathcal G(f) \in \{\mathrm{blue},\mathrm{red}\}`,

$$
d(f) = \max\big(0,\, T - V_{\mathcal G(f)}\big),\qquad B_{\rm color}(f) = 1 + \alpha\, d(f),
$$

with `α = color_alpha`. Let `w(f) ≡ cosmo_weight_by_filter[f]`. If only one
colour group has been seen so far, the first band from the other group receives
an additional nudge; write this as `n(f) ≥ 1` where, concretely, the first such
band uses `n(f) = max{1, b_first}` with `b_first = first_epoch_color_boost`, and
subsequent bands use `n(f)=1`.

- Combined utility and choice:

$$
U(f) = C_f\; w(f)\; B_{\rm color}(f)\; n(f),\qquad f^* = \underset{f\in\mathcal A\cap\{G_f=1\}}{\arg\max}\; U(f),
$$
